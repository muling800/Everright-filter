...
const getRemoteData = async () => {
  ...
  try {
    const { data } = await (isInConstraint
      ? ER.api.getPropValues(params)
      : ER.api.getConditions(params));
    state.options = data;
  } finally {
    state.loading = false;
  }
...
getRemoteData();
const { getData, setData, clearData, v$ } = hooks.useCommon(NAME.CASCADERTYPE, {
  ...toRefs(state),
  ...toRefs(props),
  isMultiple,
});
...
const handleChange = (value) => {
  if (unref(isMultiple)) {
    ...
    newValue.forEach((e) => {
      const selectedChildren = flatNodes([
        _.find(element.value.$refs.cascaderPanelRef.getCheckedNodes(), {
          valueByOption: e,
        }),
      ])
        .map((e) => e.valueByOption)
        .filter((e1) => e1 !== e);
      const findChildren = _.intersectionBy(
        element.value.$refs.cascaderPanelRef.getCheckedNodes(),
        selectedChildren.map((e) => {
          return { valueByOption: e };
        }),
        'valueByOption'
      );
...
<template>
  <div v-loading="state.loading" class="Everright-filter-el-cascader">
    <el-cascader
      ...
      :class="[
        ns.b(),
        ns.e('width'),
        v$.value0.$error && ER.props.isShowValidateState && 'ERFILTER-ERROR',
        utils.addTestId(`${NAME.CASCADERTYPE}`, 'id'),
      ]"
